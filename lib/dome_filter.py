...
def is_dome_site_raw(site_obj) -> bool:
    """Raw keyword match with no context validation.

    Any Cultural/Mixed site whose full text contains one or more FORM_KEYWORDS
    is accepted outright.  Matches the behaviour of
    tumulus_dome_evolution_raw_sweep.py (N=90, the primary non-iterative corpus).
    """
    full_text = site_obj.full_text
    return any(FORM_KEYWORD_RES[k].search(full_text) for k in FORM_KEYWORDS)
