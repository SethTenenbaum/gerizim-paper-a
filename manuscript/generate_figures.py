...
def _load_owtrad_midpoints():
    """Return list of (mid_lon, mid_lat) for each OWTRAD route edge."""
    return [((e[0] + e[2]) / 2, (e[1] + e[3]) / 2) for e in _load_owtrad_edges()]
...
def make_geo_trail():
    ...
    for hlon in [GERIZIM, java_lon]:
        ax.plot([hlon, hlon], [-14, 60], transform=ccrs.PlateCarree(),
                color="#555555", linewidth=1.4, linestyle="-",
                alpha=0.9, zorder=3)
    ...
    for _e_lon1, _e_lat1, _e_lon2, _e_lat2 in _load_owtrad_edges():
        ax.plot([_e_lon1, _e_lon2], [_e_lat1, _e_lat2],
                ...
                alpha=0.40, zorder=3,
                label="OWTRAD route edges" if _owtrad_first else "_nolegend_")
        _owtrad_first = False
    ...
    # OWTRAD edge midpoints — small purple dots
    _owtrad_mids = _load_owtrad_midpoints()
    _mid_lons = [m[0] for m in _owtrad_mids]
    _mid_lats = [m[1] for m in _owtrad_mids]
    ax.scatter(_mid_lons, _mid_lats, transform=ccrs.PlateCarree(),
               s=3, c="#7d3c98", alpha=0.60, marker="o",
               edgecolors="none", zorder=4, label="OWTRAD edge midpoints")
    ...
    tier_handles = [
        ...
        Line2D([0],[0], color="#8e44ad", linewidth=1.0, alpha=0.5,
               label="OWTRAD route edges"),
        Line2D([0],[0], marker="o", color="w", markerfacecolor="#7d3c98",
               markersize=5, label="OWTRAD edge midpoints"),
...
def make_supp_global_tiers():
    ...
    ax.plot([GERIZIM, GERIZIM], [-85, 85], transform=ccrs.PlateCarree(),
            color="#555555", linewidth=1.2, linestyle="-", alpha=0.85, zorder=3)
    ...
    handles = [
        ...
        Line2D([0],[0], marker="^", color="w", markerfacecolor="#555555",
               markersize=6,  label="▲ Wikidata Q180987 stupa"),
        # routes
        Line2D([0],[0], color="#8e44ad", linewidth=1.0, alpha=0.5, label="OWTRAD route edges"),
        Line2D([0],[0], color="#16a085", linewidth=1.4, linestyle="--",
...
def make_supp_silkroad_ac_tiers():
    ...
    handles = [
        ...
        Line2D([0],[0], marker="o", color="w", markerfacecolor="#7d3c98",
               markersize=5, label="OWTRAD edge midpoints"),
        Line2D([0],[0], color="#16a085", linewidth=1.4, linestyle="--",
               label="Maritime route (Periplus; Hansen 2012; Sen 2014)"),
    ]
    leg = ax.legend(handles=handles, loc="lower left",
                    fontsize=6.8, framealpha=0.92, ncol=1,
                    title="Harmonic tier / corpus / route", title_fontsize=7.2,
                    handletextpad=0.4, borderpad=0.55, labelspacing=0.25)
    leg.set_zorder(20)
