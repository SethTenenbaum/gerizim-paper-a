...
def main():
    ...
    # Mexico sub-corpus — use p1435_mesoamerica.csv (same source as sub-region audit)
    # Mexico bounding box: lat 14.5–29.0, lon -118.0 to -87.0
    MESO_CSV = ROOT / "data" / "store" / "wikidata" / "p1435_mesoamerica.csv"
    df_meso = pd.read_csv(MESO_CSV)
    df_meso["dev"]   = df_meso["beru_dev"].abs()
    df_meso["aplus"] = df_meso["dev"] <= TIER_AP
    mex_rows = df_meso[
        df_meso["lat"].between(14.5, 29.0) & df_meso["lon"].between(-118.0, -87.0)
    ]
    n_mex  = len(mex_rows)
    k_mex  = int(mex_rows["aplus"].sum())
    rate_mex   = k_mex / n_mex * 100 if n_mex else 0
    depl_mex   = (k_mex / n_mex) / P_NULL_AP if n_mex else 0
    res_mex    = binomtest(k_mex, n_mex, P_NULL_AP, alternative="less")
    # Mexico is ns — report actual p-value
    p_mex_str  = f"{res_mex.pvalue:.4f}" if res_mex.pvalue >= 0.001 else f"$< 0.001$"
    # sig label for Mexico
    mex_sig = "ns" if res_mex.pvalue >= 0.05 else ("*" if res_mex.pvalue >= 0.01 else "***")

    # Americas excluding Mexico (from global CSV)
    mex_mask_global = (
        am["lat"].between(14.5, 29.0) & am["lon"].between(-118.0, -87.0)
    )
    am_nomex   = am[~mex_mask_global]
    n_nomex    = len(am_nomex)
    k_nomex    = int(am_nomex["aplus"].sum())
    rate_nomex = k_nomex / n_nomex * 100 if n_nomex else 0
    depl_nomex = (k_nomex / n_nomex) / P_NULL_AP if n_nomex else 0
    res_nomex  = binomtest(k_nomex, n_nomex, P_NULL_AP, alternative="less")
    ...
    # Mexico
    print(f"  \\newcommand{{\\MexicoCtrlN}}{{{n_mex}}}          % P1435 Mexico sub-corpus size (mesoamerica.csv)")
    ...
    print(f"  \\newcommand{{\\AmericasNoMexAp}}{{{k_nomex}}}           % Americas ex-Mexico A+ hits")
    print(f"  \\newcommand{{\\AmericasNoMexRate}}{{{rate_nomex:.3f}}}         % Americas ex-Mexico A+ rate (%)")
    print(f"  \\newcommand{{\\AmericasNoMexDeplFactor}}{{{depl_nomex:.3f}}}      % Americas ex-Mexico depletion factor")
