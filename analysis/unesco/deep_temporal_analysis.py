...
print(f"  \\newcommand{{\\NdatedSites}}{{{n_with_date}}}")
print(f"  \\newcommand{{\\NpreCE}}{{{n_pre}}}")
print(f"  \\newcommand{{\\NpreCEap}}{{{nap_pre}}}")
print(f"  \\newcommand{{\\preCErate}}{{{100*pre_ce_rate:.1f}}}")
print(f"  \\newcommand{{\\preCEenrich}}{{{pre_ce_enrich:.2f}}}")
print(f"  \\newcommand{{\\pPreCEbinom}}{{{p_pre_binom:.4f}}}")
print(f"  \\newcommand{{\\NpostCE}}{{{n_post}}}")
print(f"  \\newcommand{{\\NpostCEap}}{{{nap_post}}}")
print(f"  \\newcommand{{\\postCErate}}{{{100*nap_post/n_post:.1f}}}")
print(f"  \\newcommand{{\\preCEfisherOR}}{{{or_val:.2f}}}")
print(f"  \\newcommand{{\\pPreCEfisher}}{{{p_fisher:.4f}}}")

print(f"\n  % A+ antiquity profile")
print(f"  \\newcommand{{\\NapPreCE}}{{{len(ap_pre_ce)}}}")
print(f"  \\newcommand{{\\apPreCEpct}}{{{100*len(ap_pre_ce)/len(ap_sites):.0f}}}")
print(f"  \\newcommand{{\\NapDeep}}{{{len(ap_deep)}}}")
print(f"  \\newcommand{{\\apPreCEofDated}}{{{100*len(ap_pre_ce)/len(ap_with_date):.0f}}}")
frac_non_ap_pre = len(non_ap_pre_ce)/len(non_ap_with_date) if len(non_ap_with_date) > 0 else 0
print(f"  \\newcommand{{\\nonApPreCEpct}}{{{100*frac_non_ap_pre:.0f}}}")
print(f"  \\newcommand{{\\preCEantiqFisherOR}}{{{or2:.2f}}}")
print(f"  \\newcommand{{\\pPreCEantiqFisher}}{{{p2:.4f}}}")

print(f"\n  % Spearman: founding date vs A+ status")
print(f"  \\newcommand{{\\rhoFoundDate}}{{{rho2:.4f}}}")
print(f"  \\newcommand{{\\pFoundDateSpearman}}{{{p_sp2:.4f}}}")

# Sequential/peak analysis
print(f"\n  % Sequential inscription analysis")
print(f"  \\newcommand{{\\peakSigYear}}{{{peak_year}}}")
print(f"  \\newcommand{{\\peakSigN}}{{{peak_n}}}")
print(f"  \\newcommand{{\\peakSigAp}}{{{peak_ap}}}")
print(f"  \\newcommand{{\\peakSigRate}}{{{100*peak_rate:.1f}}}")
print(f"  \\newcommand{{\\peakSigP}}{{{peak_p:.8f}}}")
print(f"  \\newcommand{{\\peakNegLogP}}{{{peak_neglog:.2f}}}")

if first_sig:
    print(f"  \\newcommand{{\\firstSigYear}}{{{first_sig['year']}}}")
    print(f"  \\newcommand{{\\firstSigN}}{{{first_sig['cum_n']}}}")

print(f"\n  % Half-corpus comparison")
print(f"  \\newcommand{{\\NfirstHalf}}{{{n1}}}")
print(f"  \\newcommand{{\\NapFirstHalf}}{{{nap1}}}")
print(f"  \\newcommand{{\\firstHalfRate}}{{{100*nap1/n1:.1f}}}")
print(f"  \\newcommand{{\\pFirstHalf}}{{{p1:.6f}}}")
print(f"  \\newcommand{{\\NsecondHalf}}{{{n2}}}")
print(f"  \\newcommand{{\\NapSecondHalf}}{{{nap2}}}")
print(f"  \\newcommand{{\\secondHalfRate}}{{{100*nap2/n2:.1f}}}")
print(f"  \\newcommand{{\\pSecondHalf}}{{{p2:.6f}}}")
print(f"  \\newcommand{{\\halfFisherOR}}{{{or3:.2f}}}")
print(f"  \\newcommand{{\\pHalfFisher}}{{{p3:.6f}}}")
